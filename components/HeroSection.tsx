...
const MOCKUPS = [
  {
    ...
    style: { transform: "perspective(1200px) rotateY(15deg) scale(0.85) translateX(-150px)", transformStyle: "preserve-3d" as const },
  },
  {
    src: "/mobile1.png",
    alt: "ScreenToSkill Challenge Interface Mockup",
    className: "relative z-20",
    style: { width: "clamp(160px, 36vw, 230px)", aspectRatio: "390/840", transform: "perspective(1200px) rotateY(0deg)" },
    priority: true,
  ...
  },
  {
    ...
    style: { transform: "perspective(1200px) rotateY(-15deg) scale(0.85) translateX(130px)", transformStyle: "preserve-3d" as const },
  },
];

const STATS = [
  { value: "100k+", label: "Active Families", desc: "Nurturing high-performance study habits and reducing device dependencies daily." },
  { value: "5M+", label: "Correct MCQ Quests", desc: "Our comprehensive curriculum empowers core skill tracking dynamically.", green: true },
  { value: "45+ Mins", label: "Saved Daily", desc: "Replaced addictive brain rot with constructive mental fitness workouts.", green: true },
];

export default function HeroSection() {
  return (
    <>
      <section id="hero" className="relative pt-32 pb-20 md:pt-24 md:pb-28 overflow-hidden bg-gradient-to-b from-slate-50 to-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
            ...
            <div className="lg:col-span-6 space-y-5 text-center lg:text-left">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-[3.5rem] xl:text-[4rem] font-display font-bold tracking-tight text-slate-900 leading-[1.15] pt-0.5 text-center lg:text-left">
                Turn Screen Time <br className="hidden sm:inline" />Into Skill Time
              </h1>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-1">
                <a href="#download" className="group relative w-full sm:w-auto px-6 py-3 bg-[#22C55E] hover:bg-[#02bd78] text-white rounded-xl text-xs sm:text-sm font-bold shadow-sm shadow-emerald-500/15 hover:shadow-emerald-500/25 transition-all text-center cursor-pointer flex items-center justify-center space-x-2 min-w-[200px]">
            ...
            <div id="install-interactive" className="lg:col-span-6 flex justify-center items-center relative select-none min-h-[440px] sm:min-h-[560px] lg:min-h-[660px] py-6 sm:py-10 overflow-visible isolate">
              <div className="absolute inset-x-0 inset-y-0 -z-30 pointer-events-none rounded-[3rem] overflow-hidden">
                <Image src="/Mobile-App-Hero-BG.webp" alt="Mobile App Hero BG" fill priority className="object-cover scale-102 opacity-100 select-none animate-fade-in" referrerPolicy="no-referrer" />
              </div>
              ...
              ].map((cls, i) => <div key={i} className={`${cls} pointer-events-none`} />)}
              ...
              <div className="relative w-full flex justify-center items-center h-[300px] sm:h-[400px] lg:h-[460px] perspective-[1200px] overflow-visible">
                {MOCKUPS.map(({ src, alt, className, style, priority, sizes }) => (
                  <div key={src} className={`overflow-hidden flex flex-col transition-all duration-500 hover:scale-99 group ${className}`} style={style}>
                    <div className="relative w-full h-full overflow-hidden">
                      <Image src={src} alt={alt} fill priority={priority} sizes={sizes ?? "185px"} className="object-cover group-hover:scale-99 transition-transform duration-500" referrerPolicy="no-referrer" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center divide-y md:divide-y-0 md:divide-x divide-slate-100">
            {STATS.map(({ value, label, desc, green }) => (
              <div key={label} className="py-2.5 md:py-0">
                <span className={`block text-2xl sm:text-3xl font-bold ${green ? "text-[#22C55E]" : "text-slate-800"}`}>{value}</span>
                <span className="text-[10px] uppercase tracking-wider font-mono text-slate-400 font-bold block mt-1">{label}</span>
                <p className="text-[11px] text-slate-400 max-w-xs mx-auto mt-1 font-normal leading-relaxed">{desc}</p>
              </div>
            ))}
          </div>
        </div>
